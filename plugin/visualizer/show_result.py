...
from .open3d_arrow import get_yaw_arrows, get_speed_arrows
...
def show_box_label(vis, bboxes, labels, show_yaw=False, show_speed=True):
    if bboxes is not None:
        if labels is None or labels.numel() == 0:
            vis.add_bboxes(bbox3d=bboxes)
        else:
            palette = np.random.randint(
                0, 255, size=(labels.max() + 1, 3)) / 256
            labelDict = {}
            for j in range(len(labels)):
                i = int(labels[j].numpy())
                if labelDict.get(i) is None:
                    labelDict[i] = []
                labelDict[i].append(bboxes[j])
            for i in labelDict:
                vis.add_bboxes(
        ...
        # 画yaw箭头
        if show_yaw and bboxes.shape[1] >= 7:
            arrows = get_yaw_arrows(pre_box_set=bboxes)
            for arrow in arrows:
                vis.o3d_visualizer.add_geometry(arrow)

        if show_speed and bboxes.shape[1] >= 9:
            arrows = get_speed_arrows(pre_box_set=bboxes)
            for arrow in arrows:
                vis.o3d_visualizer.add_geometry(arrow)
...
def show_result(points,
    ...
    Args:
        ...
        snapshot (bool, optional): Whether to save the online results.
            Defaults to False.
        labels (np.ndarray, optional): Predicted labels of boxes.
            Defaults to None.
    """
