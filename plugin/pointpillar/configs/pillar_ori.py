...
# data_root = '/mnt/data/adt_dataset/nuscenes/' #服务器
data_root = '/media/zwh/ZWH4T/ZWH/Dataset3d/nuscenes/' #本机
resume_from = None
# key Parameters      <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

# Configs of Datasets >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
point_cloud_range = [-20, -20, -5, 60, 20, 3]
...
model = dict(
    ...
    pts_middle_encoder=dict(
        type='PointPillarsScatter', in_channels=64, output_shape=[160, 320]),
    pts_backbone=dict(
        type='SECOND',
        in_channels=64,
...
# Configs of Others    >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
evaluation = dict(interval=300, pipeline=eval_pipeline)
