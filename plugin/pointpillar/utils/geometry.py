...
def label2class(class2label):
    ...
    for key in class2label:
        val = class2label[key]
        if val not in label2class.keys():
            label2class[val] = key
            class_num = class_num + 1
    return label2class, class_num
...
def bbox3d2corners(bboxes):
    ...
    rot_mat = np.concatenate(
        [rot_cos, rot_sin, np.zeros_like(rot_cos),
         ...
         np.zeros_like(rot_cos), np.zeros_like(rot_cos), np.ones_like(rot_cos)], axis=1).astype(np.float32).reshape(-1,
                                                                                                                    3,
                                                                                                                    3)
...
def box_collision_test(boxes, qboxes, clockwise=True):
    ...
    lines_boxes = np.stack((boxes, boxes[:, slices, :]),
                           axis=2)  # [N, 4, 2(line), 2(xy)] 顺时针的线段 01 12 23 30
    lines_qboxes = np.stack((qboxes, qboxes[:, slices, :]), axis=2)
    # vec = np.zeros((2,), dtype=boxes.dtype)
    boxes_standup = bevcorner2alignedbbox(boxes)  # n个box [xmin ymin xmax ymax]
    qboxes_standup = bevcorner2alignedbbox(qboxes)
    for i in range(N):
        for j in range(K):
            # calculate standup first
            '''
                       ----------         box_standup = [[xmin ymin xmax ymax]...] 
                       | qbox   |
                ---------       |       
                |  box ||       |      /|x  
                |      -|--------       |
            ...
            iw = (
                    ...
                    max(boxes_standup[i, 0], qboxes_standup[j, 0]))  # if collide possibly in x direction x方向是否碰撞
            if iw > 0:
                ih = (
                        min(boxes_standup[i, 3], qboxes_standup[j, 3]) -
                        max(boxes_standup[i, 1], qboxes_standup[j, 1]))  # if collide possibly in y direction y方向是否碰撞
                if ih > 0:
                    for k in range(4):
...
def remove_pts_in_bboxes(points, bboxes, rm=True):
    ...
    bboxes_corners = bbox3d2corners(bboxes)  # (n, 8, 3) 3dbox转8个顶点
...
def nearest_bev(bboxes):
    ...
    bboxes_bev = torch.where(torch.abs(bboxes_angle[:, None]) > np.pi / 4, bboxes_bev[:, [0, 1, 3, 2]],
                             bboxes_bev)  # l,w与xy坐标系夹角相关,与yaw解绑

    bboxes_xy = bboxes_bev[:, :2]
    bboxes_lw = bboxes_bev[:, 2:]
    bboxes_bev_x1y1x2y2 = torch.cat([bboxes_xy - bboxes_lw / 2, bboxes_xy + bboxes_lw / 2], dim=-1)
...
def iou_bev(bboxes1, bboxes2):
    ...
    bev_overlap = boxes_iou_bev(bboxes1_bev.cuda(), bboxes2_bev.cuda())  # (n, m)
...
def remove_outside_points(points, r0_rect, tr_velo_to_cam, P2, image_shape):
    """Remove points which are outside of image.  保留相机视角的椎体内的激光点云
    Args:
    ...
    frustum -= T
    frustum = np.linalg.inv(R) @ frustum.T  # 相机坐标系下的3d坐标
    frustum = points_camera2lidar(frustum.T[None, ...], tr_velo_to_cam, r0_rect)  # (1, 8, 3) #lidar坐标系下的3d坐标
    group_rectangle_vertexs_v = group_rectangle_vertexs(frustum)  # 椎体的6个面的顶点
    frustum_surfaces = group_plane_equation(group_rectangle_vertexs_v)
...
def get_frustum(bbox_image, C, near_clip=0.001, far_clip=100):
    ...
    根据四个像素顶点坐标计算锥体近处和远处横截面的8个顶点3d坐标
    Args:
